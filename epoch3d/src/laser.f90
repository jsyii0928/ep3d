...
MODULE laser
   ...
   USE custom_laser
   USE evaluator
   USE gpulse_mod, ONLY: gpulse_init, gpulse_free, gpulse
               ...
               IF (time >= current%t_start .AND. time <= current%t_end) THEN
                  DO j = 0,nz
                     DO i = 0,ny
                        ...
                        bx(laserpos-1, i, j) = bx(laserpos-1, i, j) + gp_Bx
                     END DO
                  END DO
               END IF
            END IF
               ...
               IF (time >= current%t_start .AND. time <= current%t_end) THEN
                  DO j = 0,nz
                     DO i = 0,nx
                        ...
                        by(i, laserpos-1, j) = by(i, laserpos-1, j) + gp_By
                     END DO
                  END DO
               END IF
            END IF
               ...
               IF (time >= current%t_start .AND. time <= current%t_end) THEN
                  DO j = 0,ny
                     DO i = 0,nx
                        ...
                        bz(i, j, laserpos-1) = bz(i, j, laserpos-1) + gp_Bz
                     END DO
                  END DO
               END IF
            END IF

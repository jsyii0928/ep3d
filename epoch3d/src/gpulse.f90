...
!$       if (rank == 0) print *, "[Gpulse] Rank ", rank, " Initializing Field Solver (Fortran) with OpenMP..."
         if (rank == 0) print *, "[Gpulse] Rank ", rank, " Initializing Field Solver (Fortran)..."
      ...
      !$OMP END CRITICAL(gpulse_init_lock)
   ...
   function load_cache() result(success)
      ...
      if (.not. allocated(g_field%omegas)) allocate(g_field%omegas(h_n_freq))
      if (.not. allocated(g_field%data)) allocate(g_field%data(h_n_x * h_n_y * h_n_freq))
      ...
      close(iu)
      success = .true.
      if (rank == 0) print *, '[Gpulse] Loaded field data from cache.'
   ...
   subroutine gpulse_complex(t, x, y, z, z_offset, Ex, Ey, Ez, Bx, By, Bz)
      real(dp), intent(in) :: t, x, y, z, z_offset
      ...
      theta_base = t + (z - z_offset) / C_CONST
   ...
   subroutine gpulse(t, x, y, z, z_offset, Ex, Ey, Ez, Bx, By, Bz)
      real(dp), intent(in) :: t, x, y, z, z_offset
      real(dp), intent(out) :: Ex, Ey, Ez, Bx, By, Bz
      complex(dpc) :: cEx, cEy, cEz, cBx, cBy, cBz

      call gpulse_complex(t, x, y, z, z_offset, cEx, cEy, cEz, cBx, cBy, cBz)
